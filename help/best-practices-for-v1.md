...
description: ' [!DNL Adobe Experience Manager] 桌面应用程序版本1.10的关键功能和建议使用。'
...
>[!NOTE]
>
>在阅读本文档之前，您可以查看整体[AEM和Creative Cloud集成最佳实践](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/aem-cc-integration-best-practices.html)，以了解该主题的更高级别概述。
...
* **将链接的资产放置到其他文档中**。在应用程序(如Creative Cloud（[!DNL Adobe Photoshop]、[!DNL Adobe InDesign]和[!DNL Adobe Illustrator]）中，可以将外部文件作为链接放置。 例如，您可以将图像放入InDesign文档中。 在这种情况下，网络共享装载允许您从AEM中浏览和选择资产以进行放置。 在某些非Adobe应用程序（如MS Office）中，也可以放置链接的文件。

* **AEM中的引用解析**。如果已放置的文件和具有链接的主文件都存储在AEM中，则它可以自动提供有关资产引用的服务器端信息。
...
* 请勿使用AEM桌面应用程序将大量数据从一个AEM实例迁移到另一个实例。 请参阅[迁移指南](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/assets-migration-guide.html)以规划和执行资产迁移。 相反，桌面应用程序[支持在[!DNL Adobe Experience Manager]中首次批量上传](use-app-v1.md#bulkupload)大量资产。

## Recommendations ，用于选定用例{#recommendations-for-selected-use-cases}

### 对创意用户资产的访问权限{#access-to-assets-for-creative-users}
...
* 考虑资产的正确权限（访问控制），以便为创意用户简化DAM存储库的视图，基本上限制他们仅访问他们需要或感兴趣的资产：
   ...
   * DAM中的大多数资产是最终资产，并且不用于更改 — 这些资产对于创意用户应为只读。
...
### 更新使用AEM桌面应用程序{#updating-assets-opened-using-aem-desktop-app}打开的资产
...
* **使用正确配置的Dispatcher**。使用AEM Dispatcher提高安全性，并确保已将其配置为[AEM桌面应用程序连接到调度程序](install-configure-app-v1.md#connect-to-an-aem-instance-behind-a-dispatcher)后面的AEM

* **节省带宽**。在Mac上的Finder中关闭图标预览 — 使用Finder浏览已装载的存储库时。 “查找器”请求每个文件生成一个预览，并导致桌面应用程序在本地下载和缓存资产。 请注意，在节省带宽的同时，也会减少桌面用户的用户体验，因此在使用具有大资产和/或有限带宽的存储库时，应该这样做。

>[!NOTE]
>
>要关闭图标预览，请在“访达”中转到[!UICONTROL View]，选择[!UICONTROL View Options]，然后取消选中[!UICONTROL Show icon preview]选项。 此操作仅适用于当前文件夹 — 若要使其成为默认文件夹，请单击同一对话框中的[!UICONTROL Use as default]选项。

### 优化服务器性能{#optimizing-server-performance}

要了解如何优化AEM Assets服务器的性能，请参阅[AEM Assets性能优化指南](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/performance-tuning-guidelines.html)。 AEM桌面应用程序服务器性能的一些重要方面是围绕优化工作流配置，以便其在资产上传方面表现良好：

* **性能更高的资产上传**。将[AEM资产更新工作流模型配置为临时](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/performance-tuning-guidelines.html)。

* **限制上传的服务器CPU**。确保正确设置最大并行工作流作业参数，以便上传不会耗尽所有CPU。
