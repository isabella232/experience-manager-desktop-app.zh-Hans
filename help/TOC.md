---
cloud: experience-cloud
product: Adobe Experience Manager desktop app
audience: end-user
user-guide-title: AEM 桌面应用程序指南
breadcrumb-title: Desktop App 指南
user-guide-description: 了解如何使用 AEM 桌面应用程序连接存储库和桌面应用程序，以便更快地访问资源并简化工作流。
solution-title: AEM Assets
solution-hub-url: https://docs.adobe.com/content/help/en/experience-manager-65/assets/home.html
getting-started-url: https://docs.adobe.com/content/help/zh-Hans/experience-manager-desktop-app/using/introduction.html
tutorials-title: AEM 教程
tutorials-url: https://helpx.adobe.com/cn/experience-manager/kt/index/aem-tutorials.html
translation-type: tm+mt
source-git-commit: 7b2f16b35b89707e94c24f9d420080c9f13ea945
workflow-type: tm+mt
source-wordcount: '87'
ht-degree: 95%

---


# Adobe desktop app Guide {#using}

+ [概述](introduction.md)
+ [发行说明](release-notes.md)
+ [安装或升级](install-upgrade.md)
+ [使用应用程序](using.md)
+ [故障诊断](troubleshoot.md)
+ 以前版本的帮助 {#archive}
   + [发行说明 v1](release-notes-of-v1.md)
   + [安装和配置 v1](install-configure-app-v1.md)
   + [使用应用程序 v1](use-app-v1.md)
   + [最佳实践 v1](best-practices-for-v1.md)
   + [故障诊断 v1](troubleshoot-app-v1.md)

