...
* 使用AEM Desktop的上下文菜单，用户将存回/取出资源，或返回DAM用户界面。
* 完成对文件的更改后，用户将返回到AEM Web UI

这并非唯一的用例。 但是，它说明了AEM Desktop是一种方便的本地访问/编辑资源的机制。 我们鼓励您尽可能多地使用DAM Web UI，因为它提供了更好的体验。 它为Adobe提供了更大的灵活性，以满足客户需求。

## 限制 {#limitations}

WebDAV/SMB1网络共享为在资源管理器/Finder窗口中处理文件提供了便利。 但是，Explorer/Finder和AEM通过具有某些限制的网络连接进行通信。 例如，将1 GB文件复制到已装载的WebDAV/SMB目录所耗时间与使用Web浏览器将1 GB文件上载到网站所需的时间大致相同。 事实上，在前一种情况下，持续时间可能更长，原因是WebDAV/SMB协议和操作系统的WebDAV/SMB客户端（特别是Mac OS X）效率低下。

从已装载的目录可以执行的任务类型存在限制。 通常，处理大文件（特别是在低/高延迟/低带宽网络连接上）可能具有挑战性，尤其是在编辑大文件时。

Adobe建议在向客户端提交某些类型的文件可以从安装的目录就地高效编辑之前，先执行一些用例测试。

AEM桌面不适合执行密集的文件系统操作，包括但不限于：
...
由于操作系统的限制，Windows的文件大小限制为4,294,967,295字节（约4.29 GB）。 这是由于注册表设置定义了网络共享上的文件可以有多大。 注册表设置的值是一个DWORD，其最大大小等于引用的编号。

[!DNL Experience Manager] 桌面应用程序没有可配置的超时值，该超时值在固定时间间隔后 [!DNL Experience Manager] 会断开服务器与桌面应用程序之间的连接。在上传大型资源时，如果连接在一段时间后超时，则应用程序会通过增加上传超时来重试上传资源几次。 建议不要更改默认超时设置。

## 缓存并与AEM {#caching-and-communication-with-aem}通信

AEM桌面应用程序提供内部缓存和后台上传功能，以改善最终用户体验。 保存大文件时，首先将它保存在本地，以便您继续工作。 在某个时间（当前为30秒）后，文件将在后台发送到AEM服务器。

与Creative Cloud桌面或其他文件同步解决方案（如Microsoft One Drive）不同，AEM桌面应用程序不是完整的桌面同步客户端。 原因是它提供对整个AEM Assets存储库的访问，该存储库可能非常大（数百GB或TB），以实现完全同步。

缓存功能可以将网络/存储开销限制为仅包含与用户相关的资源子集。

>[!CAUTION]
>
>Adobe建议关闭缩览图生成以加快浏览速度。 如果启用图标预览，则当您浏览已挂载的文件夹时，应用程序会缓存数字资源。 该应用程序还下载用户可能不关心的资源，这会增加服务器负载，占用用户的带宽，并使用用户的更多磁盘空间。

以下是AEM桌面应用程序执行缓存的方式：

* 当您在Finder中打开文件夹并显示文件的缩览图/预览时，或者当您在应用程序中打开文件时，桌面应用程序会缓存文件的二进制文件。
* 当您通过Finder或其他桌面应用程序存储文件时，文件首先会本地存储（缓存），并通知操作系统。 然后，文件将排队上传到后台服务器，并最终通过网络上传。 如果出现网络错误，桌面应用程序将重试上载整个文件最多三次。 如果在三次重试后上载失败，则文件被标记为冲突文件，并且状态通过“后台上载队列状态”窗口显示。 桌面应用程序不再尝试更新文件。 用户应在恢复连接后更新文件并重新上传它
...
对单个用户的未优化性能进行故障诊断时，请首先查看[应用程序限制](#limitations)。 后续部分包括改进单个用户性能的建议。
...
如果您在Windows上使用Experience Manager，则可以配置Windows以增强WebDAV客户端的性能。 有关详细信息，请访问[https://support.microsoft.com/en-us/kb/2445570](https://support.microsoft.com/en-us/kb/2445570)。
